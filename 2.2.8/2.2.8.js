const getLanguagesStatistic = (feedbacks) => {
    const statistic = {};
    const filteredByYear = feedbacks.filter((feedback) => {
        const {year} = feedback;
        return year === 2019;
    });

    filteredByYear.forEach((feedback) => {
        let {language} = feedback;
...
// использования языков программирования. На входе функция получает массив отзывов пользователей. Необходимо 
// вернуть объект в формате { languageName: count, anotherLanguageName: anotherCount, ... }, 
